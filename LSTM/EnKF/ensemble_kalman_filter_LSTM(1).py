...
class EnsembleKalmanFilter(object):
    ...
    x_queue : np.array(Ensemble size, LSTM Memory - 1, dim_x)
        Store the past states. 
    ...
    def __init__(self, x, P, dim_z, dt, N, hx, fx, ensemble_size, LSTM_seq_len, x_queue):
        if dim_z <= 0:
    ...
    def update(self, z, R=None):
        ...
        #sigmas_h = torch.clamp(torch.from_numpy(sigmas_h), min = np.atan(-15), max = np.atan(15)).detach().cpu().numpy()
        ...
        #self.sigmas = torch.clamp(torch.from_numpy(self.sigmas), min = -15, max = 15).detach().cpu().numpy()
    ...
    def predict(self):
        ...
        N = self.N
        current = deepcopy(self.sigmas)
        next_t = self.fx(self.sigmas, self.x_queue, self.dt)
        self.sigmas = deepcopy(next_t)
        self.x_queue = np.concatenate((self.x_queue[: , 1: , : ], current.reshape([N, 1, self.dim_x])), axis = 1)
        #for i, s in enumerate(self.sigmas):
            #self.sigmas[i] = self.fx(s, self.x_queue[i], self.dt)
